...
/*   Updated: 2024/09/19 22:59:29 by visaienk         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	put_sprite(t_map *map)
{
	...
	mlx_set_instance_depth(&map->mlx_assets->sprite->instances[0], 4);
}
...
void	put_exit(t_map *map)
{
	if (mlx_image_to_window(map->mlx,
			...
			map->finish_r * 50) < 0)
		ft_error("exit error\n", map);
	mlx_set_instance_depth(&map->mlx_assets->exit->instances[0], 5);
}
